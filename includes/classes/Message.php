...
class Message {
	...
	public function getLatestMessage($userLoggedIn, $user2) {
		...
		$row = mysqli_fetch_array($query);
		$sent_by = ($row['user_to'] == $userLoggedIn) ? "O: " : "Sen: ";

        //Zamanlama
        $date_time_now = date("Y-m-d H:i:s");
        $start_date = new DateTime($row['date']); //Gönderilme saati
	...
	public function getConvosDropdown($data, $limit) {
		...
		if($count > $limit)
			...
			$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'>Başka mesaj yok!</p>";
